...
- **Talkover FIFO needs chown to liquidsoap UID (10000).** Docker's AppArmor profile blocks writes to bind-mounted FIFOs by non-owner processes, even if permissions are 0666. `talkover-capture.sh` runs as uid 10000 inside the container and needs to open `/tmp/disco-talkover.pcm` with O_RDWR. Fix: `ExecStartPre` must `chown 10000:10001 /tmp/disco-talkover.pcm` before starting the container. Without this, talkover crash-loops and stalls the clock.
...
- **`btmgmt public-addr` hangs forever on the built-in BCM adapter.** When `bt-setup.sh` tried to set a stable address on the built-in (hci0, UART), `btmgmt --index 0 public-addr ...` blocked indefinitely (`|| true` can't save you — it never returns, it just hangs). The oneshot sat in `activating (start)` for hours, leaving the built-in powered off. Fix: `bt-setup.sh` prefers the USB dongle (which has a valid address and needs no btmgmt) and guards the built-in fallback's btmgmt with `timeout 5`. Check `systemctl is-active disco-bt-setup` is `active`, not `activating`.
- **`bluetoothctl devices Connected` only queries the *default* controller.** With two adapters (built-in hci0 + USB dongle hci1), a phone paired to the non-default one is invisible — the admin shows 0 BT devices even though it's connected and audio is bridging via bluealsa. Symptom: `/api/bluetooth/status` returns `{devices:[]}` while `bluealsactl list-pcms` shows the A2DP source. Fix: `server/lib/bluetooth.js` iterates `bluetoothctl list` and runs `select <ctrl>; devices Connected` on every controller, deduped by MAC.
