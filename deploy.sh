...
run_scp "$SCRIPT_DIR/config/disco.liq" "$SCRIPT_DIR/config/nginx-disco.conf" "$SCRIPT_DIR/config/bt-capture.sh" "$SCRIPT_DIR/config/bt-auto-agent.py" "$PI_USER@$PI_IP:/tmp/"
run_ssh "sudo cp /tmp/nginx-disco.conf /etc/nginx/sites-available/disco && sudo cp /tmp/bt-capture.sh /tmp/bt-auto-agent.py /opt/disco/config/ && sudo chmod +x /opt/disco/config/bt-capture.sh"
...
# Cleanup
run_ssh "rm -rf /tmp/disco-js /tmp/disco-css /tmp/disco-lib /tmp/index.html /tmp/admin.html /tmp/server.js /tmp/package.json /tmp/disco.liq /tmp/nginx-disco.conf /tmp/bt-capture.sh /tmp/bt-auto-agent.py 2>/dev/null"
